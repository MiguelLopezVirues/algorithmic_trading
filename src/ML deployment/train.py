...
def load_select_features():
    ...
    print("Data long columns", data_long_indicators_exog.columns)
...
def generate_training_dicts(date_range, training_window, data_long_indicators_series, data_long_indicators_exog):
    ...
    end = datetime.strftime(date_range[-5], "%Y-%m-%d") # Also, it should be done on weekends, but right now it is not a weekend, so the last day available day is a Thursday
...
def generate_out_of_sample_pred(date_range, training_window, transformer_exog, model):
    ...
    end_val = datetime.strftime(date_range[-5], "%Y-%m-%d")
